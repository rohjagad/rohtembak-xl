...
def admin_prices_xl_custom_pin_add(
    ...
    """Pin family code (UUID) baru — user bisa browse paket keluarga ini
    tanpa mengetik kodenya (kode pin tidak pernah ditampilkan)."""
    if user.role != "admin":
        return RedirectResponse(url="/user/dashboard", status_code=303)
...
def _norm_custom_pins(raw) -> list:
    """Normalisasi daftar family code pin (UUID) → list str. Entri legacy
    bertipe dict (label+family_code) tetap dibaca family_code-nya. Dedupe."""
    vals = raw if isinstance(raw, list) else []
    pins = []
    seen = set()
    for v in vals:
        fc = _valid_custom_family_code(str(v.get("family_code") if isinstance(v, dict) else v or ""))
